...
ATTENDEE_TABLE = os.environ['ATTENDEES_TABLE_NAME']

def get_attendees(MEETING_ID):
    input = {
        "TableName": ATTENDEE_TABLE,
        "FilterExpression": "begins_with(#409d0, :409d0)",
        "ExpressionAttributeNames": {"#409d0":"AttendeeId"},
        "ExpressionAttributeValues": {":409d0": {"S":MEETING_ID}}
    }
    attendees = dynamodb.scan(**input)
    print('List of attendees below:')
    print(attendees)
    
    attendeeIds = list(o['AttendeeId']['S'].split("/",1)[1] for o in attendees['Items'])
    print(attendeeIds)
    return attendeeIds
